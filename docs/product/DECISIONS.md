...
## D-024: Checkpoint 8B conversations and messages
- **Status:** Accepted by explicit Checkpoint 8B scope · 2026-09-25.
- **Names:** Keep MVP's canonical threads, thread_participants, messages and /messages/[threadId]. A thread is a conversation, uniquely associated with one pitch and its shared opportunity_id.
- **Participants:** Exactly the pitch sender and the Opportunity's immutable created_by profile. The creator was authorized to create the Opportunity; owner-side read/send additionally requires current admin membership in its owning organization. Other admins/members do not automatically join. Removed creator admins lose access; reassignment and team conversations are deferred. Immutable derived from_id/owner_id columns on threads allow non-recursive invoker RLS; clients cannot supply these identities or opportunity_id. Participants are atomically created by an invoker trigger.
- **Creation:** Sender explicitly opens a conversation after pitching. get_or_create_thread is SECURITY INVOKER and UNIQUE(pitch_id) resolves concurrent opens. No pitch note is copied to messages.
- **Messages:** Plain text, trimmed, 1–2000 Unicode characters. Participant-only reads/inserts, sender_id = auth.uid(), no normal UPDATE/DELETE. Each insert locks its thread before persisting, then an invoker trigger computes the match.
- **Metric:** matched_at transitions only when real persisted messages exist from both derived sides. It is set once and never overwritten. Column-limited UPDATE permission supports invoker triggers and row locking; a guard computes the timestamp from database facts and ignores client timestamps. Pitch notes, thread creation and one-sided messages do not count.
- **Deferrals:** External matched_conversation analytics emission is DEFERRED; no existing event/outbox infrastructure exists. matched_at is the persisted source of truth. Realtime, notifications, attachments, reactions, typing, read receipts, pitch workflow, Deals, Premium and quotas remain deferred. Refresh and successful-send reload provide updates.
