...
| | |
|---|---|
| Status | **Phase 0: Validation**. Website draft exists; product code frozen until "Go" (D-021). |
...
Planning docs and code are kept apart.
...
│   ├── product/          ← MVP.md, DECISIONS.md, DESIGN_SYSTEM.md
│   └── engineering/      ← TECH_STACK.md, BUILD_PLAN.md, WEBSITE_PLAN.md, OWNERSHIP.md, SHARED_CONTRACTS.md
│
├── client/               ← the Expo app: website + iOS + Android, one codebase
│                           (draft exists, frozen to Phase 0 pages until "Go"; see WEBSITE_PLAN §0)
└── supabase/             ← the backend: database migrations, tests, server functions (not created yet)
```
...
| [WEBSITE_PLAN.md](docs/engineering/WEBSITE_PLAN.md) | Site map, every page and its functions, folder structure, step-by-step website plan, freeze rule | **Now.** Approve before more code is written. |
...
| Term | Meaning |
|---|---|
| **Organizer** | An organization that runs events and looks for sponsors |
| **Sponsor** | An organization that funds or supports events (cash or in-kind) |
| **Organization** | A company page: event company, community, university club, nonprofit, company, or agency. Every account is one organization (D-025); the person who signs in is never shown |
| **Post** | The one listing type (like an Upwork job). It has two kinds: |
| ↳ **Event post** | Posted by an organizer: the event plan (dates, place, attendance, audience), the sponsorship goal, the support needed, what sponsors get, and optional priced tiers |
| ↳ **Sponsor post** | Posted by a sponsor: the events wanted, budget per event, the support given, and what is wanted in return |
| **Proposal** | An application to a post (like an Upwork proposal): a message plus an optional tier pick or amount. Statuses: `new` → `shortlisted` → `in talks` → `won` / `declined`, then `completed` after the event |
| **Proposals inbox** | Where the post owner compares applicants and moves each proposal through its statuses |
| **Review** | After a won proposal is marked **completed**, each side rates the other 1–5 stars with a written review. Reviews are public on organization pages and replace likes |
| **Follow** | Organizations follow each other; the Find page has a Following tab |
| **Boost** | Paid, labeled priority in search for 7 days (max 2 per 10 results) |
| **Matched conversation** | A thread where both an organizer and a sponsor have replied. **This is the north-star metric.** |
| **Deal** | A proposal marked **won**, then **completed** after the event |
...
We're in **Phase 0: Validation (October 2026)**. No product code gets written until the go/no-go at the end of Phase 0. Only the Phase 0 website (Home, Privacy, 404, waitlist) may be finished ([WEBSITE_PLAN §0](docs/engineering/WEBSITE_PLAN.md#0-current-state-and-freeze-rule)).
